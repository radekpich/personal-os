...
import type { Category, Context, Tag, TaskStatus, Vision } from "@/lib/api/types";
import { TaskDialog } from "@/components/tasks/task-detail-panel";

export function QuickCapture({ categories, contexts, tags, visions, dialogDefaultStatus = "todo" }: { categories: Category[]; contexts: Context[]; tags: Tag[]; visions: Vision[]; dialogDefaultStatus?: TaskStatus }) {
  ...
  return (
    <>
      ...
      <TaskDialog mode="create" open={open} initialTitle={value} defaultStatus={dialogDefaultStatus} categories={categories} contexts={contexts} tags={tags} visions={visions} onClose={() => setOpen(false)} />
    </>
  );
}
