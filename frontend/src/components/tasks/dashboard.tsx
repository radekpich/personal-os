...
export function Dashboard() {
  ...
  const cat = categories.data?.items ?? []; const ctx = contexts.data?.items ?? []; const tag = tags.data?.items ?? [];
  return (
    <div className="grid gap-6">
      ...
      <section className="grid gap-3"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">Dnešní práce</h2><Link className="text-sm text-[var(--muted)]" href="/tasks?view=today">Zobrazit vše</Link></div>{today.isLoading ? <Loading/> : <TaskList tasks={today.data?.items ?? []} categories={cat} contexts={ctx} tags={tag} onSelect={setSelected}/>}</section>
      <section className="grid gap-3"><div className="flex items-center gap-2"><ListTodo size={20}/><h2 className="text-xl font-semibold">Po termínu</h2></div>{overdue.isLoading ? <Loading/> : <TaskList tasks={overdue.data?.items ?? []} categories={cat} contexts={ctx} tags={tag} onSelect={setSelected}/>}</section>
      <TaskDetailPanel task={selected} categories={cat} contexts={ctx} tags={tag} onClose={() => setSelected(null)} />
    </div>
  );
}
