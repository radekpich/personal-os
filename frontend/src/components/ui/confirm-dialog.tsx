  ...
  return (
    <Dialog.Root open={open} onOpenChange={(next) => { if (!next) onCancel(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-x-0 bottom-0 z-50 flex h-[96dvh] max-h-[96dvh] flex-col overflow-hidden rounded-t-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface)] shadow-2xl sm:inset-y-0 sm:left-auto sm:right-0 sm:h-dvh sm:w-full sm:max-w-md sm:rounded-none sm:border-l">
          ...
          <div className="relative z-10 mt-auto flex shrink-0 justify-end gap-2 border-t border-[var(--border)] bg-[var(--surface)] p-4 pb-[calc(3.5rem+env(safe-area-inset-bottom))] sm:p-6">
